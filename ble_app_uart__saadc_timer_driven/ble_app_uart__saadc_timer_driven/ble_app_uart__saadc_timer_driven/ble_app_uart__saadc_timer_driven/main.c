...
void saadc_sampling_event_init(void)
{
 ...
 //   uint32_t ticks = SAADC_SAMPLE_RATE_DIVIDER;
	  uint32_t ticks = nrf_drv_timer_ms_to_ticks(&m_timer,25000);
...
void saadc_callback(nrf_drv_saadc_evt_t const * p_event)
{
    ...
    unsigned int message_length = 0;// message_value = 0;
		...
		int i = 0;
		float value_adc_before = 0;
		float flexADC = 0;
		float value_medium = 0;
		float value_size = 0;
		float value_mediumR = 0;
		float offset = 2;
		float offset1 = 17;
		const	float R_DIV = 10000;
		const	float vcc = 3.17;
		float over = 340282366920938480000000000000000000000.0;//340282366920938480000000000000000000000.0
//		const float total = 25;
		float value_voltage = 0;
		float value_voltage1 = 0;
		float value_voltage2 = 0;
	//	float value_adc_before = 0;
		int solandoc = 0 ;
...
////						value_adc_before = p_event->data.done.p_buffer[i];
////						for(solandoc = 0; solandoc < number; solandoc++)
////						{
////							value_adc_before += p_event->data.done.p_buffer[solandoc];
////						}
////	        printf("value_adc_before:%d\r\n", value_adc_before);
////						if(value_adc_before >= 65000)
////						{
////							value_adc_before = 0;
////						}					
////						value_adc_before /= number;
////						printf("value_adc_after:%d\r\n", value_adc_before);
////						value_medium =( 4095 / value_adc_before) -1;
////						value_mediumR = resistor / value_medium;
////							float flexV = ( 4095 / value_adc_before) - 1;
////							flexV = R_DIV  / flexV ;
////							value_voltage = (3.19 * flexV);
////							value_voltage1 = (10000 + flexV);
////							value_voltage2 =( value_voltage / value_voltage1);
////						value_size = (value_mediumR / resistor_medium);
////						if(value_size == over )
////						{
////							value_size = 0;
////						}				
////						value_size *= offset; 
////					value_medium = value_adc_before * VCC / 4095.0 ;
////					value_mediumR  = resistor * (VCC / value_medium - 1);
////						value_voltage = value_mediumR * VCC / value_mediumR + resistor ;
////						printf("Channel [%d] value_S: %0.0f\r\n", m_adc_channel_enabled, value_mediumR);
////						printf("Channel [%d] value_S: %0.0f cm \r\n", m_adc_channel_enabled, value_size);
////							printf("Channel [%d] value: %0.0f  \r\n", m_adc_channel_enabled, flexV);
////							printf("Channel [%d] value: %0.2f V  \r\n", m_adc_channel_enabled, value_voltage2);
////					//	printf("Channel [%d] value_R: %0.0f Ohm | value_size: %0.3f cm\r\n", m_adc_channel_enabled, value_mediumR, value_size);
////						message_length = sprintf(buffer,"channel[%d]=%0.2fcm\n ", m_adc_channel_enabled, value_size);
////					//	message_length = sprintf(buffer,"channel[%d]= %0.0f\r\n ", m_adc_channel_enabled, value_mediumR);
...
////						value_adc_before /= number;
////						value_medium = 4095 / value_adc_before - 1;
////						value_medium = 10000 / value_medium;
////						printf("Channel [%d] value: %0.0f\r\n", m_adc_channel_enabled, value_medium);
////						message_length = sprintf(buffer,"channel[%d]= %0.0f\r\n ", m_adc_channel_enabled, value_medium);
////						ble_nus_string_send(&m_nus,(uint8_t *)&buffer,message_length); 
////						nrf_delay_ms(650);
//					//	value_adc_before = p_event->data.done.p_buffer[i];
////						printf("Channel %d value: %d\r\n", m_adc_channel_enabled, p_event->data.done.p_buffer[i]);
////						message_length = sprintf(buffer,"channel [%d]= %d\r\n ", m_adc_channel_enabled, p_event->data.done.p_buffer[i]);
////						ble_nus_string_send(&m_nus,(uint8_t *)&buffer,message_length); 
////						value_adc_before = p_event->data.done.p_buffer[i];
////						if(value_adc_before >=65000)
////						{
////							value_adc_before = 0;
////						}
////						printf("Channel [%d] value: %d\r\n", m_adc_channel_enabled, value_adc_before);
////						message_length = sprintf(buffer,"channel [%d]= %d\r\n ", m_adc_channel_enabled, value_adc_before);
////						ble_nus_string_send(&m_nus,(uint8_t *)&buffer,message_length); 
////						nrf_delay_ms(650);
//				}
...
////						int solandoc = 0 ;
////						for(solandoc = 0; solandoc < 15; solandoc++)
////						{
////							value_adc_before += p_event->data.done.p_buffer[solandoc];
////						}
////						if(value_adc_before >= 65000)
...
//				}
        ...
        APP_ERROR_CHECK(err_code);	
				for (i = 0; i < 5; i++) 
        {
              value_adc_before += p_event->data.done.p_buffer[i]; // take N samples in a row
        }
        value_adc_before /= 5;
//				 value_adc_before = p_event->data.done.p_buffer[i]; 
				float flexV = ( 4095 / value_adc_before) - 1;
				...
				value_voltage2 =( value_voltage / value_voltage1);
				printf("Channel [%d] value: %0.0f  \r\n", m_adc_channel_enabled, flexV);
				printf("Channel [%d] value_V: %0.02f  \r\n", m_adc_channel_enabled, value_voltage2);
//				value_size = (flexV / resistor_medium);////
//				printf("Channel [%d] value_S1: %0.0f ohm \r\n", m_adc_channel_enabled, flexV);
//				printf("Channel [%d] value: %0.2f mV \r\n", m_adc_channel_enabled, value_voltage2);
//				printf("Channel [%d] value_Z: %0.0f cm \r\n", m_adc_channel_enabled, value_size);
//						for(solandoc = 0; solandoc < number; solandoc++)
//						{
//							value_adc_before += p_event->data.done.p_buffer[solandoc];
//						}
...
////						}
//						value_adc_before /= number;
//						printf("value_adc_before:%0.0f\r\n", value_adc_before);
//						value_medium =( 4095 / value_adc_before) - 1;
//						value_mediumR = resistor / value_medium;
//					//	value_voltage = value_mediumR * VCC / value_mediumR + resistor ;
//						printf("Channel [%d] value_R: %0.0f  \r\n", m_adc_channel_enabled, value_mediumR);
//						printf("\r\n");
//				message_length = sprintf(buffer,"channel [%d]= %d\r\n ", m_adc_channel_enabled, p_event->data.done.p_buffer[i]);
//        ble_nus_string_send(&m_nus,(uint8_t *)&buffer,message_length); 
//				if(m_adc_channel_enabled == 5)
//				{
//						value = (float)p_event->data.done.p_buffer[0];
//						value = (value * 2.5) / 4095;
//						printf("value voltage VDC: %0.2f\r\n", value);
//						message_value = sprintf(buffer_1, "Volgate = %0.2f\r\n", value);
//						ble_nus_string_send(&m_nus, (uint8_t *)&buffer_1, message_value); 
//				}
			//	nrf_delay_ms(10);
        m_adc_evt_counter++;
...
//    saadc_config.resolution = NRF_SAADC_RESOLUTION_12BIT;
    ...
    channel_0_config.reference  = NRF_SAADC_REFERENCE_INTERNAL ;// NRF_SAADC_REFERENCE_VDD4 //NRF_SAADC_REFERENCE_INTERNAL
    ...
    channel_1_config.pin_p      = NRF_SAADC_INPUT_AIN2;////(nrf_saadc_input_t)
    ...
    channel_2_config.pin_p      = NRF_SAADC_INPUT_AIN4;
    ...
    channel_3_config.pin_p      = NRF_SAADC_INPUT_AIN5;
    ...
    channel_4_config.pin_p      = NRF_SAADC_INPUT_AIN6;//(nrf_saadc_input_t)
    ...
    channel_5_config.pin_p      = NRF_SAADC_INPUT_AIN7;
...
//		NRF_SAADC->SAMPLERATE |= (0x50 << SAADC_SAMPLERATE_CC_Pos);  
